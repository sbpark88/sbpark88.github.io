...
설명한 것처럼 `Array 에 이미 적합성을 준수하는 구현이 존재하기 때문에 Extension 을 이용해 Protocols 를 채택하고 비워두는 것 만으로 
...
이제 Stack 의 `==(lhs:rhs:) -> Bool` 메서드는 더이상 Protocol 의 요구조건을 준수하기 위해 구현을 강요받지 않는다. 따라서 
...
그리고 우리는 이런 *constraints* 를 이용해 조건부로 적용할 요구 조건을 Extensions 이 아닌 Context 내에 정의할 수도 있다. 
...
*Container* Protocol 에서 Iterable 관련 요구 조건을 분리해 별도의 *IterableContainer* Protocol 로 만들고,
[Contextual Where Clauses][Contextual Where Clauses] 대신 
[Extensions with a Generic Where Clause][Extensions with a Generic Where Clause] 를 사용하도록 변경했다.
...
역시 다음과 같이 [Extensions with a Generic Where Clause][Extensions with a Generic Where Clause] 로 
변경할 수 있다.
...
> **IterableContainer** Protocol 을 정의한 것처럼 다양한 요구 조건을 Container Protocol 을 준수하는 다양한 하위 
...
의 요구 조건을 준수하도록 하기 위해 *Stack* 은 Int Type 의 단일 index 를 받아 반환하는 코드를 구현했다.
...
*Container* Protocol 의 요구 조건은 `subscript(i: Int) -> Item { get }`이었기 때문에 *subscript*는 `Int` Type 
만 Parameters 로 허용된다. 따라서 `[2...5]`와 같은 `Sequence`는 사용할 수가 없다.

`Parameters 를 Int Type 의 Sequence 로 받는 subscript`를
[Protocol Extensions - Providing Default Implementations][Protocol Extensions - Providing Default Implementations] 
를 이용해 추가적으로 제공해보자.
