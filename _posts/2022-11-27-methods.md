...
`anotherMoveBy(x:y:)`는 `self`, 즉, `Instance` 자기 자신을 `Point(x:y)`를 이용해 새 
`Instance`를 생성한 후, 기존의 `Instance`를 대체한다.
...
그리고 과연 이때 `new Instance`를 이용해 `original Instance`를 대체하면 `somePoint`의 
메모리 주소가 바뀌는지도 함께 확인해 보았으나 변경되지 않는 것으로 보인다.
...
> `Type Methods`에서 `self`는 `Instance`가 아닌 `Type itself`, 즉, `Type` 자체를 
> 가리킨다.
> 
...
>   또한 Type Methods 는 Instance 의 생성과는 무관하므로, Type 의 full name 을 붙여 접근 가능하다.
...
- `complete(level:)` : 현재 레벨의 게임을 완료하면 `LevelTracker`의 `unlock)_:)` 
  메서드를 이용해 게임의 최고 레벨을 해제하고, 현재 게임 레벨을 올린다.

> 여기서 주목해야 할 점은 `LevelTracker`는 `Structure`고, `Player`는 `Class`라는 
> 점이다. 또한 게임 설정의 `LevelTracker`를 보면 `highestUnlockedLevel`는 
> `Type Property`고, `currentLevel`은 `Instance Property`다.  
> 즉, 어떤 플레이어가 게임의 최고 레벨을 높여 놓으면 그 다음 플레이어는 해당 레벨을 잠금이 
> 해제된 게임을 이용할 수 있음을 의미하며, 최고 레벨 해제와 무관하게 새 플레이어는 항상 
> 레벨 1로 게임을 시작해야함을 의미한다.
...
이번에는 헤르미온느가 게임을 시작하는데, 헤르미온느는 해리가 하던 1번방에서 게임을 이어서 하기로 하고 
플레이어를 새로 만들어 게임에 들어갔다. 1번 방에서 해리는 레벨 2를 진행하고 있었지만 헤르미온느는 
새 게임을 시작하는 플레이어이므로 게임 설정 `LevelTracker`에 의해 마찬가지로 레벨 1에서 시작한다.
...
그런데 헤르미온느는 낮은 레벨의 게임은 재미 없다며 한 번에 레벨 6으로 올리기를 원했다. 
현재 게임 플레이가 가능한 최고 레벨이 2라는 것을 확인 했으나 혹시나 하는 마음에 레벨 6으로 
올리기를 시도했고, 레벨 6은 아직 잠겨있다는 메시지를 받았다.
