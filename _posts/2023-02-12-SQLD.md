...
![ERD Relationship](/assets/images/posts/2023-02-12-SQLD/erd-relationship.PNG){: width="1000"}
...
### 10. Connect by (계층형 조회) 👩‍💻

`계층형` 또는 `재귀`로 불리며 다음과 같은 특징을 갖는다.
...
![Grouping](/assets/images/posts/2023-02-12-SQLD/grouping.png){: width="800"}
...
![Grouping Sets](/assets/images/posts/2023-02-12-SQLD/grouping-sets.png){: width="800"}

#### 4. CUBE

- 조합할 수 있는 경우의 수를 모두 조합.

![Cube](/assets/images/posts/2023-02-12-SQLD/cube.png){: width="800"}

---

### 13. Window Functions 👩‍💻

`행의 위치를 조작`하는 함수를 `Window Functions`라 한다.
...
![Window Functions](/assets/images/posts/2023-02-12-SQLD/window-functions.png){: width="800"}

![Window Function FIRST_VALUE](/assets/images/posts/2023-02-12-SQLD/window-function-first-value.png){: width="800"}

![Window Function LAST_VALUE](/assets/images/posts/2023-02-12-SQLD/window-function-last-value.png){: width="800"}

![Window Function LAG](/assets/images/posts/2023-02-12-SQLD/window-function-lag.png){: width="800"}

![Window Function LEAD](/assets/images/posts/2023-02-12-SQLD/window-function-lead.png){: width="800"}
...
| NTILE(n) | 특정 컬럼을 `n 등분`한 결과를 조회.<br>`1 ~ n` 등급 정수 반환. |
| RATIO_TO_REPORT | 파티션 별로 `SUM 에 대한 값의 지분 백분율` 조회.<br>`0 ~ 1` 사이를 반환. |

![Window Function CUME_DIST](/assets/images/posts/2023-02-12-SQLD/window-function-cume-dist.png){: width="800"}

![Window Function PERCENT_RANK](/assets/images/posts/2023-02-12-SQLD/window-function-percent-rank.png.png){: width="800"}

> 같은 부서에서 자신의 급여 퍼센트(등수)를 구한다.

![Window Function NTILE](/assets/images/posts/2023-02-12-SQLD/window-function-ntile.png){: width="800"}

> `NTILE(4)`, 즉 4개로 등분하여 분류한다. (i.e. 급여가 높은 순으로 4개로 등분한다)

![Window Function RATIO_TO_REPORT](/assets/images/posts/2023-02-12-SQLD/window-function-ratio-to-report.png){: width="800"}
...
- 옵티마이저는 `Data Dictionary`에 있는 오브젝트 통계, 시스템 통계 등의 정보를 사용해 예상되는 비용을 산정하고 
  `Execution Plan(실행 계획)`을 수립하고 SQL을 실행한다.
- 최저 비용을 가지는 계획을 산정해서 SQL을 실행한다.
- `PLAN_TABLE`을 조회해 실행 계획을 확인할 수 있다.
- 옵티마이저가 비효율적인 실행 계획을 수립하면 `힌트(HINT)` 를 사용해 실행 계획 변경을 요청할 수 있다.

> Q) A 테이블, B 테이블이 있고 `AND 연산(INNER JOIN)`을 하려고 한다. 이때 A 테이블이 B 테이블보다 더 크다. 어떤 순서로 연산을 해야 
> 비용이 더 적을까?  
...
> 💡`최신 오라클`은 규칙 기반 옵티마이저보다 `비용 기반 옵티마이저`를 `default`로 사용한다.
...
High Watermark : 테이블에 `데이터`가 저장된 블록에서 `최상위 위치`를 의미한다.
