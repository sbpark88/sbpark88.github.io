...
![SQL JOINS](/assets/images/posts/2023-02-12-SQLD/sql-joins.png){: width="1000"}
...
- `GROUP BY` 컬럼에 대해서 `Sub total`을 만들어준다.
- `GROUP BY` 컬럼구에 `두 개 이상`이 오면 `순서`에 따라 `결과가 달라진다`.
...
![Grouping 1](/assets/images/posts/2023-02-12-SQLD/grouping-1.png){: width="1000"}

![Grouping 2](/assets/images/posts/2023-02-12-SQLD/grouping-2.png){: width="1000"}
...
![Grouping Sets](/assets/images/posts/2023-02-12-SQLD/grouping-sets.png){: width="1000"}

#### 4. CUBE

- 조합할 수 있는 경우의 수를 모두 조합.

![Cube](/assets/images/posts/2023-02-12-SQLD/cube.png){: width="1000"}

---

### 13. Window Functions 👩‍💻

윈도우 함수는 `행과 행 간의 관계를 비교, 연산, 정의하기 위한 함수`로 연산을 통해 행의 집계를 나타내거나 순서를 조작하는 등의 작업에 사용된다.
...
| ORDER BY     | 정렬                                                                           |
| WINDOWING    | 행 기준 범위를 정한다. <br> `ROWS` : `물리적 결과`의 `행 수` <br> `RANGE` : `논리적인 값`에 의한 `범위` |
...
동점자가 존재할 때 *RANK*, *DENSE_RANK*, *ROW_NUMBER* 는 서로 다른 순위를 출력한다.

| 함수         | 결과                  | 설명                    |
|------------|---------------------|-----------------------|
| RANK       | 1, 2, 2, 2, 5, 6, 7 | `동일 순위`는 `동일한 값`      |
| DENSE_RANK | 1, 2, 2, 2, 3, 4, 5 | `동일 순위`를 `하나의 건수로 계산` |
| ROW_NUMBER | 1, 2, 3, 4, 5, 6, 7 | `고유의 순위`를 부여          |
...
빈칸을 주고 *SUM*, *AVG*, *COUNT*, *MAX*, *MIN* 중 맞는 것을 찾는 문제가 주로 출제되는 것 같다.

#### 5. 행 순서 관련 함수

__1 ) FIRST_VALUE(=MIN)__
...
![Window Functions Order 1](/assets/images/posts/2023-02-12-SQLD/window-function-order-1.png){: width="1000"}

![Window Functions Order 2](/assets/images/posts/2023-02-12-SQLD/window-function-order-2.png){: width="1000"}

![Window Functions Order 3](/assets/images/posts/2023-02-12-SQLD/window-function-order-3.png){: width="1000"}

![Window Functions Order 4](/assets/images/posts/2023-02-12-SQLD/window-function-order-4.png){: width="1000"}

#### 6. 비율 관련 함수

| 명령어             | 설명                                                                                       |
|-----------------|------------------------------------------------------------------------------------------|
| CUME_DIST       | 파티션 별로 최댓값 대비 `값의 백분율` 조회.<br>`0 ~ 1` 사이를 반환.                                            |
| PERCENT_RANK    | 파티션 별로 `행의 시작을 0`, `행의 마지막을 1`로 해서 `행 순서로 등수의 백분율` 조회(값에 의한 백분율 아님). <br>`0 ~ 1` 사이를 반환. |
| NTILE(n)        | 특정 컬럼을 `n등분`한 결과를 조회.<br>`1 ~ n` 등급 정수 반환.                                               |
| RATIO_TO_REPORT | 파티션 별로 `SUM에 대한 값의 지분 백분율` 조회.<br>`0 ~ 1` 사이를 반환.                                        |

![Window Functions Radio 1](/assets/images/posts/2023-02-12-SQLD/window-function-ratio-1.png){: width="1000"}

![Window Functions Radio 2](/assets/images/posts/2023-02-12-SQLD/window-function-ratio-2.png){: width="1000"}

![Window Functions Radio 3](/assets/images/posts/2023-02-12-SQLD/window-function-ratio-3.png){: width="1000"}

![Window Functions Radio 4](/assets/images/posts/2023-02-12-SQLD/window-function-ratio-4.png){: width="1000"}

![Window Functions Radio 5](/assets/images/posts/2023-02-12-SQLD/window-function-ratio-5.png){: width="1000"}

![Window Functions Radio 6](/assets/images/posts/2023-02-12-SQLD/window-function-ratio-6.png){: width="1000"}
...
- 옵티마이저는 `Data Dictionary`에 있는 오브젝트 통계, 시스템 통계 등의 정보를 사용해 예상되는 비용을 산정하고 `Execution Plan(실행 계획)`을 수립하고 SQL을 실행한다.
- 최저 비용을 가지는 계획을 산정해서 SQL 을 실행한다.
- `PLAN_TABLE`을 조회해 실행 계획을 확인할 수 있다.
- 옵티마이저가 비효율적인 실행 계획을 수립하면 `힌트(HINT)` 를 사용해 실행 계획 변경을 요청할 수 있다.

> Q) A 테이블, B 테이블이 있고 AND 연산(INNER JOIN)을 하려고 한다. 이 때 A 테이블이 B 테이블보다 더 크다. 어떤 순서로 연산을 해야 비용이 더 적을까?
> 
> A) `더 작은 B 테이블을 먼저 읽고 A 테이블과 비교` 해야 비용이 적게 든다.

#### 2. Optimizer Order

`SQL` > `Pasing` > `Optimizer` from Data Dictionary > `Execution Plan` on PLAN_TABLE > 
`SQL execute` > `Fetch`
...
| 엔진                  | 설명                               |
|---------------------|----------------------------------|
| `Query Transformer` | `SQL`을 효율적으로 실행하기 위해 `옵티마이저가 변환` |
| `Estimator`         | `통계` 정보를 사용해 실행 `비용을 계산`         |
| `Plan Generator`    | SQL `실행 계획(Execution Plan) 수립`   |

<br>

- `규칙 기반 옵티마이저` 15개 우선 순위

전체 이론은 구글 검색하길 바라며 시험에 연관될만한 암기 항목만 정리한다.
...
```sql
SELECT /*+ RULE */  -- 규칙 기반 옵티마이저를 사용하도록 힌트를 사용.
FROM EMP
WHERE ROWID = 'AAAAHABDDAAJFNKANBJ';  -- ROWID 값으로 행을 검색.
```

<br>

- `비용 기반 옵티마이저` : 오브젝트 통계 및 시스템 통계를 사용해서 총 비용을 계산

> 💡`최신 오라클`은 규칙 기반 옵티마이저보다 `비용 기반 옵티마이저`를 기본으로 사용한다.
...
- `기본키(Primary Key)`는 자동으로 `인덱스`가 만들어지고 인덱스의 이름은 `SYSXXXX`이다.
- `인덱스 키`로 `정렬`되어 있기 때문에 `빠른 조회`가 가능하다.
- 인덱스는 `오름차순` 및 `내림차순` 탐색이 가능하다.
- 하나의 테이블에 `여러 개의 인덱스를 생성`할 수 있고 `하나의 인덱스`는 `여러 개의 컬럼으로 구성될 수 있다`.
- Index 는 Root Block, Branch Block, Leaf Block 으로 구성  
  `Branch Block` : `Pointer`로 되어있다.  
  `Leaf Block` : `정렬된 인덱스 키`와 `ROWID`로 구성. `Double Linked List`로 되어 `양방향 탐색`이 가능.
- `오라클`은 `null`을 인덱스 `맨 뒤`에 저장하고, `SQL Server`는 `맨 앞`에 저장한다.
...
> 위 인덱스 생성을 설명하면 다음과 같다.
> 
> `IND_EMP` : 인덱스 이름  
> `EMP` : 테이블 이름  
> `ENAME ASC`, `SAL DESC` : ENAME 은 오름차순, SAL 은 내림차순으로 인덱스 생성

#### 2. Index Scan

__1 ) Index Unique Scan__

인덱스 키 값이 중복되지 않는 경우

<br>

__2 ) Index Range Scan__

LIKE, BETWEEN 등을 이용해 Leaf Block 의 특정 범위를 스캔

<br>

__3 ) Index Full Scan__

검색되는 인덱스 키가 많은 경우 Leaf Block 의 처음부터 끝가지 전체를 스캔

> High Watermark : 테이블에 `데이터`가 저장된 블록에서 `최상위 위치`를 의미

#### 3. Optimizer Join

__1 ) Nested Loop Join__

- 하나의 테이블에서 데이터를 먼저 찾고 다음 테이블을 조인.
- `먼저 조회`하는 테이블을 `Outer table`, `나중에 조회`하는 테이블을 `Inner table` 이라고 한다.
- `Random Access`가 많이 발생.

<br>

__2 ) Sort Merge Join__

- `SORT_AREA`라는 `메모리 공간`에 모두 `로딩` 하고 `Sort를 수행`.
...
- 두 테이블 중 `작은 테이블을 Hash 메모리에 로딩`하고 두 테이블을 조인키를 이용해 해시 테이블을 생성.
- `CPU 연산`을 많이 한다.
- 선행 테이블이 충분히 메모리에 로딩되는 크기여야한다.
- `조인 컬럼 인덱스가 존재하지 않을 경우에도 사용 가능`.
- `=` 동등 조건에서만 사용 가능.
- 해시 테이블 저장시 `메모리 초과` ⇒ `임시 영역(디스크)에 저장`.
