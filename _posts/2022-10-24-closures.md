...
> `Swift`와 마찬가지로 함수의 `arguments`로 전달된 `Closures`의 `{ }`는 생략 가능하다. 하지만 
> `TypeScript`에서는 `argumnets`로 전달된 `Closures`의 `arguments` 자체를 생략하지는 못 한다.
...
#### <span style="color: rgba(166, 42, 254, 1)">1. `Function Declarations` with `function` / `func` keyword</span>
...
#### <span style="color: rgba(166, 42, 254, 1)">2. `Function Expressions` with `Lambda expressions` / `Closures`</span>
...
#### <span style="color: rgba(166, 42, 254, 1)">3. `Lambda expressions` / `Closures`</span>
...
#### <span style="color: rgba(166, 42, 254, 1)">4. `Autoclosures`</span>
...
> `Autoclosures`처럼 완전히 평문을 적듯이 작성하는 것은 불가능하다. 즉 `Swift`와 같은 완전한 `Autoclosures` 
> 개념에 일치하는 문법은 없다.
> 
...
> ```
> 
> `Closures`의 `Body`를 감싸는 `{ }`를 `Single Line`일 경우 생략할 수 있는 `TypeScript`와 달리 
> `Swift`에서는 생략할 수 없다.  
> `Swift`에서 `arguments`를 생략할 수 있으나 `TypeScript`에서는 반드시 필요했던 것과는 상반되는 결과다.
> 
> 즉, 이것은 어떤 언어가 더 우수하다는 것이 아닌 언어에 따른 특성이니 이를 잘 이해하고 사용하는 것이 필요하다.

#### <span style="color: rgba(166, 42, 254, 1)">5. Capturing Values with `Closures`</span>

`Functions`는 `Closures`의 특별한 형태로 위 1~3번 모두 `Closures`다. 또한 `Swift`의 `Closures`는 다른 
언어의 `Lambda expressions`와 비슷하기 때문에 단순히 `Functions`로 보일 수 있지만 `Swift` 뿐 아니라 함수를 
일급 객체로 취급하는 함수형 프로그래밍 언어의 특성을 말하는 것으로 다른 언어와 비교할 때 단순히 함수로 보이는 위 1~3번 
모두 `Clousres`의 범주에 포함된다.

하지만 일반적으로 `Closures`를 설명할 때 사용되는 개념은 다음과 같다. `값을 캡쳐`하고, 함수가 `return value`로 
또 다른 `함수를 만들어 내는 것`.  
위 1~3번과 달리 `Closures`의 특징이 생략되지 않은 케이스는 아래와 같다.
