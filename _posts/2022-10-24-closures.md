...
함수의 `argument`로 `Closure`를 전달한다. 이때 함수의 `arguments`, `return types`, `arrow(->)`은 생략했다.
...
결과가 성공일 경우 `Function Body`의 `if` statement 를 통해 사용자에게 사진을 보여줄 것이다. 
두 번째 `Closure` `() -> Void`는 `error handler`로 실패 했을 경우 `else` clause 를 통해 
...
이때 `Parameter Types` 앞에 `@autoclosure` 키워드를 붙여주면 `arguments`를 받을 때 
